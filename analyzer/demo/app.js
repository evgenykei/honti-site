...
let project,index=0,cy,positions={},selected=null,focus=null,timer=null,types=new Set(),diff,current,displayed,loading=false;
...
function details(){
 ...
 const state=diff[kind==='node'?'nodes':'edges'].get(item.id)||'same';box.append(el('span',words[state],state),el('h3',item.label||'Зависимость'),el('code',item.id));
 ...
 }else{add('Из объекта',item.source);add('В объект',item.target);add('Тип связи',item.type||'dependency');}
 box.append(dl);
 if(state==='changed'){
...
function render(){
 ...
 const elems=[...displayed.nodes.map(n=>({group:'nodes',data:{...n,color:palette[n.type]||'#b4c6cc',state:diff.nodes.get(n.id)||'same'},position:positions[n.id]})),...displayed.edges.map(e=>({group:'edges',data:{...e,state:diff.edges.get(e.id)||'same'}}))];
 ...
 $('empty').hidden=displayed.nodes.length>0;$('count').textContent=displayed.nodes.length+' объектов · '+displayed.edges.length+' связей';
 ...
 $('rows').replaceChildren();for(const n of displayed.nodes){const row=el('tr'),cell=el('td'),b=el('button',n.label);b.onclick=()=>select('node',n.id);cell.append(b,el('small',n.id));row.append(cell,el('td',project.types?.[n.type]||n.type),el('td',words[diff.nodes.get(n.id)||'same'],diff.nodes.get(n.id)));$('rows').append(row);}
 if(!displayed.nodes.length){const tr=el('tr'),td=el('td','Нет объектов по выбранным условиям.');td.colSpan=3;tr.append(td);$('rows').append(tr);}
...
function drawTypes(){
 ...
 for(const t of new Set(project.snapshots.flatMap(s=>s.nodes.map(n=>n.type)))){const label=el('label',undefined,'check'),input=el('input');input.type='checkbox';input.checked=types.has(t);input.dataset.type=t;input.onchange=()=>{stop();input.checked?types.add(t):types.delete(t);render();};label.append(input,el('span',project.types?.[t]||t),el('span',counts[t]||0,'total'));$('types').append(label);}
}
async function load(p){
 if(loading)throw Error('Дождитесь завершения текущей загрузки.');validate(p);stop();loading=true;$('import').disabled=true;$('demo').disabled=true;$('play').disabled=true;message('Рассчитываю расположение объектов…');
 ...
 project=p;positions=Object.fromEntries((layout.children||[]).map(n=>[n.id,{x:n.x+72.5,y:n.y+31}]));index=0;selected=null;focus=null;types=new Set([...ns.values()].map(n=>n.type));$('search').value='';$('time').max=p.snapshots.length-1;$('time').disabled=p.snapshots.length===1;$('ghosts').checked=true;
...
async function init(){
 ...
 cy.on('tap','node',e=>select('node',e.target.id()));cy.on('tap','edge',e=>select('edge',e.target.id()));cy.on('dragfree','node',e=>positions[e.target.id()]={...e.target.position()});
 $('deselect').onclick=()=>{selected=null;cy.elements().unselect();details();};
 $('search').oninput=()=>{stop();if(project)render();};$('direction').onchange=()=>{stop();if(project)render();};$('ghosts').onchange=()=>{stop();if(project)render();};
 $('focus').onclick=()=>{if(selected?.kind==='node'){stop();focus=selected.id;$('search').value='';render();cy.fit(undefined,45);}};
 $('unfocus').onclick=()=>{stop();focus=null;render();cy.fit(undefined,45);};
 $('reset').onclick=()=>{if(!project)return;stop();focus=null;$('search').value='';types=new Set(project.snapshots.flatMap(s=>s.nodes.map(n=>n.type)));drawTypes();render();cy.fit(undefined,45);};
